...
def multiple_rule(number, keys):
  ''' return true if number is a multiple of a the key'''
  for key in keys:
    if (number % key) != 0:
      return False
  return True

def match_rule(number, keys):
  ''' return true if number is the same as the key '''
  for key in keys:
    # instead of using regexp, count the occurence in a string
    if str(number).count(str(key)):
      return True
  return False
...
def process_rules(number,rules):
  ...
  rule_matched = False
  rule_weight = 0
  # check the number against each rule
  # this have to be replaced by the call of a function with the same name as the rule type, ex rule['type']() - don't have the time for it
  for rule in rules:
    if rule['type'] == "multiple" and rule['weight'] >= rule_weight:
      if multiple_rule(number,rule["keys"]):
        rule_result = rule["result"]
        rule_matched = True
        rule_weight = rule['weight']
    if rule['type'] == "match" and rule['weight'] >= rule_weight:
      if match_rule(number,rule["keys"]):
        rule_result = rule["result"]
        rule_matched = True
        rule_weight = rule['weight']
